...
// Storage helpers
const STORAGE_PREFIX = "mealtracker:";

async function saveMeals(dateKey, meals) {
  try {
    await window.storage.set(`${STORAGE_PREFIX}${dateKey}`, JSON.stringify(meals));
  } catch (e) {
...
async function loadMeals(dateKey) {
  try {
    const result = await window.storage.get(`${STORAGE_PREFIX}${dateKey}`);
...
async function getAllSavedDates() {
  try {
    ...
    if (result && result.keys) {
      return result.keys.map(k => k.replace(STORAGE_PREFIX, ""));
    }
    return [];
  } catch (e) {
    return [];
    ...
    <div ref={ref} style={{
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2, textAlign: "center" }}>
        ...
        {cells.map((day, i) => {
          ...
          const hasMeals = savedDates.includes(dateKey);
          return (
...
// Scrolling number picker (0–60), swipe/scroll to change
function ScrollNumberPicker({ value, onChange }) {
  ...
  const prev2 = clamp(value - 2);
  const prev1 = clamp(value - 1);
  const next1 = clamp(value + 1);
  const next2 = clamp(value + 2);
  ...
  return (
    <div
      ...
      style={{
        ...
        cursor: "ns-resize", userSelect: "none", position: "relative",
        touchAction: "none"
      }}
    >
      {/* Fade overlays */}
...
// Water intake section: single picker + OK button → adds to running total
function WaterIntakeSection({ values, onChange }) {
  const [pickerValue, setPickerValue] = useState(0);
  const [flash, setFlash] = useState(false);
  // values is just a plain number (the total) stored as waterIntake
  ...
  return (
    <div style={{
      ...
      border: "1px solid #2d5a2d", borderRadius: 16, padding: "0", overflow: "hidden",
      ...
      <div style={{
        ...
        <h3 style={{
          margin: 0, color: "#8fce8f", fontFamily: "'Playfair Display', serif",
          fontSize: 18, fontWeight: 600, letterSpacing: "0.5px"
        }}>Water Intake</h3>
        {/* Running total on the right */}
        <div style={{
          marginLeft: "auto", display: "flex", alignItems: "baseline", gap: 4,
          ...
          <span style={{
            fontSize: 11, color: "#5a8a5a", fontFamily: "'DM Sans', sans-serif",
            fontWeight: 600
          }}>oz total</span>
        </div>
      </div>
...
function MealSection({ section, items, onChange, onAdd }) {
  ...
  const handleChange = (idx, value) => {
    ...
    newItems[idx] = value;
    // Auto-add new line if typing in last field
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{
          marginLeft: "auto", fontSize: 11, color: "#5a8a5a",
        ...
        {items.map((item, idx) => (
          <div key={idx} style={{ display: "flex", alignItems: "center", gap: 8, padding: "2px 0" }}>
            <span style={{
              ...
              background: item.trim() ? "#4CAF50" : "rgba(76,175,80,0.2)",
...
export default function MealTracker() {
  ...
  useEffect(() => {
    getAllSavedDates().then(dates => setSavedDates(dates));
  }, []);
    ...
    saveTimer.current = setTimeout(async () => {
      ...
      if (hasContent) {
        ...
        if (!savedDates.includes(dateKey)) {
          setSavedDates(prev => [...prev, dateKey]);
        }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderRadius: "50%", background: "radial-gradient(circle, rgba(56,142,60,0.03) 0%, transparent 70%)",
        pointerEvents: "none"
      }} />
      ...
      <header style={{
        ...
        <h1 style={{
          ...
          fontSize: 28, fontWeight: 700, color: "#7ddf7d",
          letterSpacing: "1px"
        }}>
          Meal Tracker
        </h1>
      ...
      <div style={{
        display: "flex", alignItems: "center", justifyContent: "center",
        gap: 12, padding: "20px 24px", flexWrap: "wrap",
        animation: "fadeIn 0.4s ease 0.1s both"
      }}>
        {/* Calendar pill with date */}
        <div style={{ position: "relative" }}>
          <button onClick={() => setCalOpen(!calOpen)} style={{
            ...
            {isToday && <span style={{
              fontSize: 9, background: "#4CAF50", color: "#fff",
        ...
        <div style={{
          ...
          <button onClick={() => changeDay(-1)} style={{
            ...
            borderRight: "1px solid rgba(76,175,80,0.2)", display: "flex",
          ...
          <span style={{
            padding: "10px 16px", fontSize: 13, color: "#8fce8f",
            fontWeight: 500, minWidth: 40, textAlign: "center",
          ...
          <button onClick={() => changeDay(1)} style={{
            background: "transparent", border: "none", color: "#7ddf7d",
            padding: "10px 16px", cursor: "pointer", fontSize: 16,
            borderLeft: "1px solid rgba(76,175,80,0.2)", display: "flex",
            alignItems: "center", transition: "background 0.15s ease"
          }}
          onMouseEnter={e => e.target.style.background = "rgba(76,175,80,0.15)"}
          onMouseLeave={e => e.target.style.background = "transparent"}
        ...
        {saveStatus && (
          <span style={{
            ...
            animation: "fadeIn 0.2s ease", display: "flex", alignItems: "center", gap: 4
          }}>
      ...
      {/* Meal sections */}
      <div style={{
        maxWidth: 520, margin: "0 auto", padding: "0 20px 40px",
        ...
        {loading ? (
          ...
          <>
            <div style={{ animation: "slideUp 0.3s ease 0s both" }}>
              <WaterIntakeSection
                values={meals.waterIntake || 0}
                onChange={(val) => updateSection("waterIntake", val)}
              />
            </div>
      ...
      <div style={{
        ...
        color: "#3a6a3a", fontSize: 11, fontWeight: 500
      }}>
